export const productList = [
  {
    id: 1,
    image: "./img/productlList/6.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 340000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 2,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 390000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
  {
    id: 3,
    image: "./img/productlList/5.png",
    title: "Mèo phi hành gia SpaceX Mèo phi hành gia SpaceX",
    price: 360000,
    quantity: 20,
    desciption: `Ngoài công dụng chính làm đèn ngủ thì em mèo phi hành Space X còn là món đồ decor trang trí khiến căn phòng bạn hấp dẫn và thú vị hơn gấp nhìu lần. 

      Đây cũng là một món quà bất ngờ dành tặng người thân, thay bạn gửi lời chúc ngủ ngon mỗi ngày <3`,
    size: ["156x117x117mm"],
    material: "ABS+ PET + Silicon an toàn cho sức khỏe và môi trường",
    other: {
      batteryCapacity: "1200mAh",
      desciption: `Có 4 mức sáng khác nhau và thay đổi được nhiều màu.`,
    },
  },
  {
    id: 4,
    image: "./img/productlList/0.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 420000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 5,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 510000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
  {
    id: 6,
    image: "./img/productlList/5.png",
    title: "Mèo phi hành gia SpaceX Mèo phi hành gia SpaceX",
    price: 600000,
    quantity: 20,
    desciption: `Ngoài công dụng chính làm đèn ngủ thì em mèo phi hành Space X còn là món đồ decor trang trí khiến căn phòng bạn hấp dẫn và thú vị hơn gấp nhìu lần. 

      Đây cũng là một món quà bất ngờ dành tặng người thân, thay bạn gửi lời chúc ngủ ngon mỗi ngày <3`,
    size: ["156x117x117mm"],
    material: "ABS+ PET + Silicon an toàn cho sức khỏe và môi trường",
    other: {
      batteryCapacity: "1200mAh",
      desciption: `Có 4 mức sáng khác nhau và thay đổi được nhiều màu.`,
    },
  },
  {
    id: 7,
    image: "./img/productlList/0.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 120000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 8,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 110000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
  {
    id: 9,
    image: "./img/productlList/5.png",
    title: "Mèo phi hành gia SpaceX Mèo phi hành gia SpaceX",
    price: 50000,
    quantity: 20,
    desciption: `Ngoài công dụng chính làm đèn ngủ thì em mèo phi hành Space X còn là món đồ decor trang trí khiến căn phòng bạn hấp dẫn và thú vị hơn gấp nhìu lần. 

      Đây cũng là một món quà bất ngờ dành tặng người thân, thay bạn gửi lời chúc ngủ ngon mỗi ngày <3`,
    size: ["156x117x117mm"],
    material: "ABS+ PET + Silicon an toàn cho sức khỏe và môi trường",
    other: {
      batteryCapacity: "1200mAh",
      desciption: `Có 4 mức sáng khác nhau và thay đổi được nhiều màu.`,
    },
  },
  {
    id: 10,
    image: "./img/productlList/0.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 700000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 11,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 750000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
  {
    id: 12,
    image: "./img/productlList/5.png",
    title: "Mèo phi hành gia SpaceX Mèo phi hành gia SpaceX",
    price: 750000,
    quantity: 20,
    desciption: `Ngoài công dụng chính làm đèn ngủ thì em mèo phi hành Space X còn là món đồ decor trang trí khiến căn phòng bạn hấp dẫn và thú vị hơn gấp nhìu lần. 

      Đây cũng là một món quà bất ngờ dành tặng người thân, thay bạn gửi lời chúc ngủ ngon mỗi ngày <3`,
    size: ["156x117x117mm"],
    material: "ABS+ PET + Silicon an toàn cho sức khỏe và môi trường",
    other: {
      batteryCapacity: "1200mAh",
      desciption: `Có 4 mức sáng khác nhau và thay đổi được nhiều màu.`,
    },
  },
  {
    id: 13,
    image: "./img/productlList/0.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 340000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 14,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 390000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
  {
    id: 15,
    image: "./img/productlList/5.png",
    title: "Mèo phi hành gia SpaceX Mèo phi hành gia SpaceX",
    price: 360000,
    quantity: 20,
    desciption: `Ngoài công dụng chính làm đèn ngủ thì em mèo phi hành Space X còn là món đồ decor trang trí khiến căn phòng bạn hấp dẫn và thú vị hơn gấp nhìu lần. 

      Đây cũng là một món quà bất ngờ dành tặng người thân, thay bạn gửi lời chúc ngủ ngon mỗi ngày <3`,
    size: ["156x117x117mm"],
    material: "ABS+ PET + Silicon an toàn cho sức khỏe và môi trường",
    other: {
      batteryCapacity: "1200mAh",
      desciption: `Có 4 mức sáng khác nhau và thay đổi được nhiều màu.`,
    },
  },
  {
    id: 16,
    image: "./img/productlList/0.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 340000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 17,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 390000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
  {
    id: 18,
    image: "./img/productlList/5.png",
    title: "Mèo phi hành gia SpaceX Mèo phi hành gia SpaceX",
    price: 360000,
    quantity: 20,
    desciption: `Ngoài công dụng chính làm đèn ngủ thì em mèo phi hành Space X còn là món đồ decor trang trí khiến căn phòng bạn hấp dẫn và thú vị hơn gấp nhìu lần. 

      Đây cũng là một món quà bất ngờ dành tặng người thân, thay bạn gửi lời chúc ngủ ngon mỗi ngày <3`,
    size: ["156x117x117mm"],
    material: "ABS+ PET + Silicon an toàn cho sức khỏe và môi trường",
    other: {
      batteryCapacity: "1200mAh",
      desciption: `Có 4 mức sáng khác nhau và thay đổi được nhiều màu.`,
    },
  },
  {
    id: 19,
    image: "./img/productlList/0.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 340000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 20,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 390000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
  {
    id: 21,
    image: "./img/productlList/5.png",
    title: "Mèo phi hành gia SpaceX Mèo phi hành gia SpaceX",
    price: 360000,
    quantity: 20,
    desciption: `Ngoài công dụng chính làm đèn ngủ thì em mèo phi hành Space X còn là món đồ decor trang trí khiến căn phòng bạn hấp dẫn và thú vị hơn gấp nhìu lần. 

      Đây cũng là một món quà bất ngờ dành tặng người thân, thay bạn gửi lời chúc ngủ ngon mỗi ngày <3`,
    size: ["156x117x117mm"],
    material: "ABS+ PET + Silicon an toàn cho sức khỏe và môi trường",
    other: {
      batteryCapacity: "1200mAh",
      desciption: `Có 4 mức sáng khác nhau và thay đổi được nhiều màu.`,
    },
  },
  
];

export const _newestProducts = [
  {
    id: 1,
    image: "./img/productlList/6.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 340000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 2,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 390000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
  {
    id: 3,
    image: "./img/productlList/5.png",
    title: "Mèo phi hành gia SpaceX Mèo phi hành gia SpaceX",
    price: 360000,
    quantity: 20,
    desciption: `Ngoài công dụng chính làm đèn ngủ thì em mèo phi hành Space X còn là món đồ decor trang trí khiến căn phòng bạn hấp dẫn và thú vị hơn gấp nhìu lần. 

      Đây cũng là một món quà bất ngờ dành tặng người thân, thay bạn gửi lời chúc ngủ ngon mỗi ngày <3`,
    size: ["156x117x117mm"],
    material: "ABS+ PET + Silicon an toàn cho sức khỏe và môi trường",
    other: {
      batteryCapacity: "1200mAh",
      desciption: `Có 4 mức sáng khác nhau và thay đổi được nhiều màu.`,
    },
  },
  {
    id: 4,
    image: "./img/productlList/0.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 340000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 5,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 390000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
];

export const _weeklyBests = [
  {
    id: 1,
    image: "./img/productlList/6.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 340000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 2,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 390000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
  {
    id: 3,
    image: "./img/productlList/5.png",
    title: "Mèo phi hành gia SpaceX Mèo phi hành gia SpaceX",
    price: 360000,
    quantity: 20,
    desciption: `Ngoài công dụng chính làm đèn ngủ thì em mèo phi hành Space X còn là món đồ decor trang trí khiến căn phòng bạn hấp dẫn và thú vị hơn gấp nhìu lần. 

      Đây cũng là một món quà bất ngờ dành tặng người thân, thay bạn gửi lời chúc ngủ ngon mỗi ngày <3`,
    size: ["156x117x117mm"],
    material: "ABS+ PET + Silicon an toàn cho sức khỏe và môi trường",
    other: {
      batteryCapacity: "1200mAh",
      desciption: `Có 4 mức sáng khác nhau và thay đổi được nhiều màu.`,
    },
  },
  {
    id: 4,
    image: "./img/productlList/0.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 340000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 5,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 390000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
  {
    id: 6,
    image: "./img/productlList/5.png",
    title: "Mèo phi hành gia SpaceX Mèo phi hành gia SpaceX",
    price: 360000,
    quantity: 20,
    desciption: `Ngoài công dụng chính làm đèn ngủ thì em mèo phi hành Space X còn là món đồ decor trang trí khiến căn phòng bạn hấp dẫn và thú vị hơn gấp nhìu lần. 

      Đây cũng là một món quà bất ngờ dành tặng người thân, thay bạn gửi lời chúc ngủ ngon mỗi ngày <3`,
    size: ["156x117x117mm"],
    material: "ABS+ PET + Silicon an toàn cho sức khỏe và môi trường",
    other: {
      batteryCapacity: "1200mAh",
      desciption: `Có 4 mức sáng khác nhau và thay đổi được nhiều màu.`,
    },
  },
  {
    id: 7,
    image: "./img/productlList/0.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 340000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 8,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 390000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
  {
    id: 9,
    image: "./img/productlList/5.png",
    title: "Mèo phi hành gia SpaceX Mèo phi hành gia SpaceX",
    price: 360000,
    quantity: 20,
    desciption: `Ngoài công dụng chính làm đèn ngủ thì em mèo phi hành Space X còn là món đồ decor trang trí khiến căn phòng bạn hấp dẫn và thú vị hơn gấp nhìu lần. 

      Đây cũng là một món quà bất ngờ dành tặng người thân, thay bạn gửi lời chúc ngủ ngon mỗi ngày <3`,
    size: ["156x117x117mm"],
    material: "ABS+ PET + Silicon an toàn cho sức khỏe và môi trường",
    other: {
      batteryCapacity: "1200mAh",
      desciption: `Có 4 mức sáng khác nhau và thay đổi được nhiều màu.`,
    },
  },
  {
    id: 10,
    image: "./img/productlList/0.png",
    title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
    price: 340000,
    quantity: 10,
    desciption:
      "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
    size: ["15x12x10cm"],
    material:
      "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
    other: {},
  },
  {
    id: 11,
    image: "./img/productlList/4.png",
    title:
      "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
    price: 390000,
    quantity: 50,
    desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.

    Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
    size: ["35cm", "35cm", "55cm"],
    material: "Nỉ và bông gòn cao cấp.",
    other: {},
  },
  {
    id: 12,
    image: "./img/productlList/5.png",
    title: "Mèo phi hành gia SpaceX Mèo phi hành gia SpaceX",
    price: 360000,
    quantity: 20,
    desciption: `Ngoài công dụng chính làm đèn ngủ thì em mèo phi hành Space X còn là món đồ decor trang trí khiến căn phòng bạn hấp dẫn và thú vị hơn gấp nhìu lần. 

      Đây cũng là một món quà bất ngờ dành tặng người thân, thay bạn gửi lời chúc ngủ ngon mỗi ngày <3`,
    size: ["156x117x117mm"],
    material: "ABS+ PET + Silicon an toàn cho sức khỏe và môi trường",
    other: {
      batteryCapacity: "1200mAh",
      desciption: `Có 4 mức sáng khác nhau và thay đổi được nhiều màu.`,
    },
  },
];

export const _cart = [
  // {
  //   id: 1,
  //   image: "./img/productlList/6.png",
  //   title: "Đèn ngủ cảm ứng hình Mây AmiCute Đèn ngủ cảm ứng hình Mây AmiCute",
  //   price: 340000,
  //   quantityInCart: 5,
  //   desciption:
  //     "Ngoài công dụng chính là chiếc đèn bảo vệ giấc ngủ của bạn mỗi tối, AmiCute còn có công dụng phụ là để decor phòng thêm lung linh hơn và giúp bạn giảm stress bằng cách đấm bóp thoải mái chiếc mây này nhen.",
  //   size: ["15x12x10cm"],
  //   material:
  //     "Silicone mềm mại, kết cấu đàn hồi, không bị biến dạng, hạn chế bám bẩn",
  //   other: {},
  // },
  // {
  //   id: 2,
  //   image: "./img/productlList/4.png",
  //   title:
  //     "Mèo Quạo Phiên Bản Chị Ong Nâu Nâu Mèo Quạo Phiên Bản Chị Ong Nâu Nâu",
  //   price: 390000,
  //   quantityInCart: 4,
  //   desciption: `Mèo Quạo phiên bản chị ong nâu nâu đang làm mưa làm gió tại nhà Mi.
  //   Nhìn mặt thì quạo chút xíu thui nhưng ôm hay tựa lưng đều thoải mái lắm lun nhen. Em này mà mang đi tặng thì Mi đảm bảo là nửa kia sẽ ưng ngay từ cái nhìn đầu tiên luôn nè. `,
  //   size: ["35cm", "35cm", "55cm"],
  //   material: "Nỉ và bông gòn cao cấp.",
  //   other: {},
  // },
];

export const _themeDatas = {
  isLightTheme: true,
  lightTheme: {
    backgroundColor: "#fefefe",
    color: "black",
    colorBlur: '#444',
    boxShadowColor: "rgba(0, 0, 0, 0.2)",
    boxShadowColor1: "rgba(0, 0, 0, 0.3)",
  },
  darkTheme: {
    backgroundColor: "#222",
    color: "#eee",
    colorBlur: '#bbb',
    boxShadowColor: "rgba(0, 0, 0, 0.4)",
    boxShadowColor1: "rgba(0, 0, 0, 0.6)",
  },
};
